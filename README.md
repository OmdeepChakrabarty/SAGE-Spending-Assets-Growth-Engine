...
transactions when you ask it to, all by writing and executing its own SQL. It runs on Cloudflare Workers AI
and ships with a Gradio chat interface that streams the agent's reasoning and tool calls live, step by step.
...
no dashboard or admin panel to maintain.

The project follows the same small, explicit, single-file style as
[samwit/smolagents_examples](https://github.com/samwit/smolagents_examples) — one model, a small set of
plain `@tool` functions, no manager/agent hierarchies, no ORM layer between the agent and the database.
...
in the transcript, so a "why did it say that?" question is always answerable by scrolling up.

## Project layout

```
main.py          Gradio chat app + the CodeAgent definition and system instructions
...
- `CLOUDFLARE_ACCOUNT_ID` — from the Cloudflare dashboard (Workers AI section)
- `CLOUDFLARE_API_KEY` — an API token with Workers AI permissions
- `MODEL_NAME` — defaults to `cloudflare/@cf/meta/llama-3.1-8b-instruct`; swap in
  `cloudflare/@cf/meta/llama-3.3-70b-instruct-fp8` for a stronger (slower) model. Because SAGE now has
  write access to the database, the stronger model is worth considering even outside pure benchmark
  chasing — it's noticeably more careful about scoping `WHERE` clauses correctly.
...
Open the URL Gradio prints (default `http://localhost:7860`) and start asking questions, e.g.:
...
3. Every successful write reports the number of rows affected, which the agent is instructed to surface
   ...
   which row changed and to what value, not just a silent "done."
...
`main.py`'s `stream_agent_response()` function consumes that generator directly and turns each event into
one or more `gr.ChatMessage` bubbles, which `gr.ChatInterface` renders as they arrive — this is what you're
seeing as "writing SQL...", "running query...", "found N rows..." messages appear one after another instead
of the whole answer popping in at once. Concretely:
...
- If a chart was generated, the saved PNG is attached as an image bubble once the run finishes.

This is a direct, trimmed-down use of smolagents' own `stream_to_gradio` / `GradioUI` pattern
(`smolagents/gradio_ui.py` in the installed package) — nothing custom-built on top of it.

## Security notes

SAGE's write path is intentionally minimal, and intentionally *not* hardened for anything beyond local,
single-user use:
...
- There is no authentication, no per-user data isolation, and no audit log beyond the Gradio chat
  transcript itself and whatever the terminal/container logs capture.
...
`.env` is already in `.gitignore`, so your Cloudflare credentials won't be committed. `transactions.db` is
...
- `CLOUDFLARE_API_KEY`
- `MODEL_NAME` (optional — `zerops.yaml`/`.env.example` default is used otherwise)
...
- starts the app with `python3 main.py`, listening on port `7860`
